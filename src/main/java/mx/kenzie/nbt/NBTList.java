...
public final class NBTList implements NBTValue<List<NBT>>, NBT, List<NBT> {
    ...
    public NBTList(InputStream stream) throws IOException {
        ...
        switch (tag) {
            case BYTE -> {for (int i = 0; i < length; i++) this.add(new NBTByte(stream));}
            case SHORT -> {for (int i = 0; i < length; i++) this.add(new NBTShort(stream));}
            case INT -> {for (int i = 0; i < length; i++) this.add(new NBTInt(stream));}
            case LONG -> {for (int i = 0; i < length; i++) this.add(new NBTLong(stream));}
            case FLOAT -> {for (int i = 0; i < length; i++) this.add(new NBTFloat(stream));}
            case DOUBLE -> {for (int i = 0; i < length; i++) this.add(new NBTDouble(stream));}
            case BYTE_ARRAY -> {for (int i = 0; i < length; i++) this.add(new NBTByteArray(stream));}
            case STRING -> {for (int i = 0; i < length; i++) this.add(new NBTString(stream));}
            case LIST -> {for (int i = 0; i < length; i++) this.add(new NBTList(stream));}
            case COMPOUND -> {for (int i = 0; i < length; i++) this.add(new NBTCompound(stream));}
            case INT_ARRAY -> {for (int i = 0; i < length; i++) this.add(new NBTIntArray(stream));}
            case LONG_ARRAY -> {for (int i = 0; i < length; i++) this.add(new NBTLongArray(stream));}
